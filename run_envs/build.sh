...
#psql -h ${POSTGRES_HOST} -p ${POSTGRES_PORT} -U ${POSTGRES_USER} -d ${DATABASE} -c "CREATE SCHEMA moja;"
...
psql -h ${POSTGRES_HOST} -p ${POSTGRES_PORT} -U ${POSTGRES_USER} -d ${DATABASE} -f create_blocks.sql
psql -h ${POSTGRES_HOST} -p ${POSTGRES_PORT} -U ${POSTGRES_USER} -d ${DATABASE} -t -c "SELECT to_jsonb(t.*) - 'geom' FROM import.departamentos_blocks t" > data/input/blocks.json
