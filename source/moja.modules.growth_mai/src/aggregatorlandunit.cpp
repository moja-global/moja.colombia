...
void AggregatorLandUnit::onTimingShutdown() {
   try {
      ...
      for (auto& flux : fluxes_lu_) {
         flux_results.accumulate({local_domain_id, short(flux.date_record_id), int(location_non_temporal_id),
      ...
      for (const auto& stock : stock_values_lu_) {
         stock_results.accumulate(
             {local_domain_id, short(stock.date_record_id), int(location_non_temporal_id), short(stock.pool_idx)},
             {stock.value});
      }
      stock_values_lu_.clear();
   } catch (...) {
